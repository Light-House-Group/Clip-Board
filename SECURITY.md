...
Since 1.3.0 there is also a **Mac App Store edition**, which *is* sandboxed (the store requires it). Rather than ship an auto-paste flow that fails silently inside the sandbox, that edition compiles the feature out entirely under the `APPSTORE` flag — it copies to the clipboard and you press ⌘V. Its entitlements live in [`Clip Board/Clip Board-AppStore.entitlements`](Clip%20Board/Clip%20Board-AppStore.entitlements): only `app-sandbox = true`, with **no** network, device, or personal-information keys. Those unused capabilities are *omitted*, not set to `false` — the Mac App Store's sandbox validator rejects any sandbox entitlement whose value is `false` (Guideline 2.4.5(i)). This is the one place the explicit-`false`-for-transparency posture the direct edition uses (below) must **not** be copied. The rest of this section describes the direct edition.
